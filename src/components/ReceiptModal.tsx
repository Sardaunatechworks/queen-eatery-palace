      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar print:overflow-visible print:px-8 print:py-6">
          ...
          <div className="text-center space-y-2 pb-2">
            <div className="w-14 h-14 bg-indigo-600 text-white rounded-2xl flex items-center justify-center mx-auto mb-4 print:w-12 print:h-12 border-4 border-indigo-50 shadow-sm print:shadow-none">
               <ShoppingBag size={28} className="print:size-6" />
            </div>
            <h1 className="text-xl font-black text-dark tracking-tighter uppercase print:text-lg">The Queen's Eatery Palace</h1>
            <div className="space-y-0.5">
              <p className="text-[10px] text-gray-500 font-bold flex items-center justify-center gap-1.5">
                <MapPin size={10} className="text-primary" /> Behind Dutse Emirs House, Opposite Glo Office
              </p>
              <p className="text-[10px] text-gray-500 font-bold flex items-center justify-center gap-1.5">
                <Phone size={10} className="text-primary" /> 09155290102
              </p>
            </div>
          </div>
          ...
          <div className="border-y border-dashed border-gray-200 py-4 grid grid-cols-2 gap-y-4 text-xs print:py-3 print:gap-y-3">
            ...
            <div className="text-right space-y-0.5">
              ...
              <p className="font-bold text-dark">
                {order.createdAt?.toDate 
                  ? format(order.createdAt.toDate(), "dd/MM/yyyy h:mm a") 
            ...
            <div className="space-y-0.5">
              ...
              <p className="font-bold text-gray-700">{order.cashierName || "Staff User"}</p>
            </div>
            <div className="text-right space-y-0.5">
              <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none">Payment Mode</p>
              <span className="inline-block bg-primary/10 text-primary px-2 py-0.5 rounded font-black text-[10px] uppercase border border-primary/10">
          ...
          <div className="space-y-2 border-t border-gray-100 pt-4">
            ...
            <div className="flex justify-between items-center bg-indigo-50/50 p-3 rounded-xl print:bg-white print:p-0 print:pt-2">
              <span className="text-sm font-black text-gray-600 uppercase tracking-tighter">Grand Total</span>
              <span className="text-xl font-black text-indigo-700">{formatNaira(order.total)}</span>
            </div>
          </div>
          ...
          <div className="text-center space-y-3 pt-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-gray-50 border border-gray-100 rounded-lg text-[9px] font-bold text-gray-500 uppercase tracking-widest print:bg-white">
               {order.deliveryType === 'delivery' ? <MapPin size={10} /> : <CheckCircle2 size={10} />}
        ...
        <div className="p-6 bg-gray-50 border-t border-gray-100 flex gap-3 print:hidden">
          {mode === 'cashier' ? (
            <button
              onClick={handlePrint}
              className="flex-1 bg-indigo-600 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-200 flex items-center justify-center gap-2 active:scale-95"
            >
          ...
          <button className="bg-white text-gray-400 p-4 rounded-2xl border border-gray-200 hover:text-indigo-600 transition-colors">
            <Share2 size={18} />
          </button>
        </div>
      </motion.div>
